...
public class YoRPG {
	...
	s = "Choose which character you would like to be\n";
	...
	try {
		int x = Integer.parseInt( in.readLine() );
			if (x==1){pat = new Warrior ();}
			if (x==2){pat = new Monster ();}
			if (x==3){pat = new KingArthur ();}
			if (x==4){pat = new SirLancelot ();}
			if (x==5){pat = new Mague ();}
			if (x==6){pat = new Rogue ();}
			else {pat = new Warrior("Default");}
	}
	catch ( IOException e ) { }
